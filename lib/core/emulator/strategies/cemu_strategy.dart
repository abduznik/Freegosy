...
class CemuStrategy extends EmulatorStrategy {
  ...
  Future<void> launch(Game game, String romPath) async {
    ...
    if (io.Platform.isLinux && exePath.endsWith('.sh')) {
      await Process.start('bash', [exePath, '-g', romPath], mode: ProcessStartMode.detached);
    } else {
  ...
  Future<Process?> launchWithHandle(Game game, String romPath) async {
    ...
    if (io.Platform.isLinux && exePath.endsWith('.sh')) {
      return await Process.start('bash', [exePath, '-g', romPath], mode: ProcessStartMode.normal);
    }
