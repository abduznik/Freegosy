...
- `lib/core/emulator/linux_strategies/` — SteamOS/Linux environment strategies.
  - `linux_environment_strategy.dart` — Interface for ROM/Save/Tool path resolution on Linux. `splitCommand()` resolves `flatpak` to its absolute path (`resolveFlatpakExecutable()`, issue #84); compare with `isFlatpakExecutable()`, never `== 'flatpak'`.
...
- `lib/core/platform/window_service.dart` — Desktop window control via `window_manager` (issue #74): `init(fullscreen:)` from main(), `toggleFullScreen()` (F11 in app.dart), `--fullscreen` flag, `launch_fullscreen` pref. No-op off desktop, never throws.
...
- `lib/ui/widgets/cover_size_button.dart` — Library app-bar button + dialog with a cover-size slider (issue #59). Drives `columnCountProvider` inversely within `kMinColumnCount..kMaxColumnCount` (2..12).
